...
class Point(_Point):
    ...
    def rotate(self, center, angle):
        ...
        x = (
            math.cos(angle) * (self.x - center.x)
            - math.sin(angle) * (self.y - center.y)
            + center.x
        )
        y = (
            math.sin(angle) * (self.x - center.x)
            + math.cos(angle) * (self.y - center.y)
            + center.y
        )
        return Point(x, y)
...
def getRotatedRectanglePoints(angle, base_point, height, width):
    ...
    ur = Point(width * math.cos(angle), -width * math.sin(angle))
    lr = Point(ur.x + height * math.sin(angle), ur.y + height * math.cos(angle))
    ll = Point(
        height * math.cos(math.pi / 2 - angle), height * math.sin(math.pi / 2 - angle)
    )
...
def dispCrop(img, CV_coords=None, PV_coords=None, crop_info=None):
    if crop_info is None:
        ...
        asinus_dist = np.linalg.norm(PV_coords - CV_coords).astype(int)

        # compute angle between portal and central veins relative to coordinate axes
        asinus_theta = np.arctan(
            (CV_coords[1] - PV_coords[1]) / (CV_coords[0] - PV_coords[0])
        )
        ...
        box = patches.Rectangle(
            ...
            1.4 * asinus_dist,
            0.4 * asinus_dist,
            angle=np.rad2deg(asinus_theta),
            rotation_point=(CV_coords[0], CV_coords[1]),
            alpha=0.5,
        )
        base = box.get_corners()[2]
        base = Point(*base)
        points = getRotatedRectanglePoints(
    ...
    cropped_im = _cropWithPoints(img, crop_info["angle"], crop_info["points"])
...
def getSampleAsinus(organelle_dir, CV_coords, PV_coords, asinusNum):
    ...
    sample_asinus, crop_info = dispCrop(
        sample_asinus_slice, CV_coords, PV_coords[asinusNum, :]
    )
...
def getDAPI(nuclei_dir, nSlices, stackNum, crop_info):
    ...
    zstack = np.zeros((nSlices, crop_info["height"], crop_info["width"]), dtype="uint8")
    print(zstack.shape)
    ...
    for i in range(nSlices):
        ...
        if asinus.shape > zstack[i].shape:
            ...
            asinus = np.pad(
                asinus,
                (
                    (0, zstack.shape[1] - asinus.shape[0]),
                    (0, zstack.shape[2] - asinus.shape[1]),
                ),
                "constant",
                constant_values=0,
            )
...
def getOrgStacks(organelle_dir, channels, nSlices, stackNum, crop_info):
    asinus_maxproj = np.zeros(
        (len(channels), crop_info["height"], crop_info["width"]), dtype="uint8"
    )
...
def makeAcinusDir(lobule_dir, asinusNum):
    ...
    if not os.path.exists(acinus_dir):
        os.umask(0)
        os.mkdir(acinus_dir, mode=0o777)
    else:
