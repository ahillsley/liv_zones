...
from liv_zones import organelle_pancreas as org
...
def cell_props(
    ...
    for index, cell in enumerate(cell_props["label"]):
        ...
        if large_peroxisome_props is not None:
            ...
            cell_props["large_peroxisome_density"][index] = organelles.density("large_peroxi")
            cell_props["large_peroxisome_avg_area"][index] = organelles.avg_area("large_peroxi")
            cell_props["large_peroxisome_perimeter"][index] = organelles.avg_perimeter("large_peroxi")
            ...
            cell_props["large_peroxisome_circularity"][index] = organelles.avg_circularity("large_peroxi")
            cell_props["large_peroxisome_solidity"][index] = organelles.solidity("large_peroxi")
...
#this one is the old one from liver and does not work for pancrease where there are no lipid_droplets 
...
def dist_from_edge(cell, organelle_list):
    # --- Minimal guards (add these only) ---
        if organelle_list is None or len(organelle_list) == 0:
            return 0.0

        try:
            cx = organelle_list["centroid-0"]
            cy = organelle_list["centroid-1"]
            bd = organelle_list["boundry_dist"]
        except KeyError:
        # missing expected columns -> cannot compute distance
            return np.nan
    # --- end minimal guards ----------------
